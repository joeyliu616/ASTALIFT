...
function productListCtrl($scope,$http,$log,notify){
    ...
    $http.get(urlFactory('/products.json')).then(
    ...
    $scope.add2Cart = function (item){
        ...
        $http.post(urlFactory("/buy/cart/" + item.id + ".json"), cartItem).then(
...
function shoppingCartCtrl($scope, $http, $log, SweetAlert){
    ...
    $http.get(urlFactory('/buy/cart/list_all.json')).then(
        function successCallback(responseData){
    ...
    $scope.removeItem = function(productId){
        ...
        $http.delete(urlFactory("/buy/cart/"+productId + ".json")).then(
            function successCallback(responseData){
                setItems(responseData.data);
            },
            function errorCallback(responseData){
    ...
    $scope.saveCartSuccess = function () {
        $http.put(urlFactory("/buy/cart.json",$scope.items)).then(
            function successCallback(responseData){
                SweetAlert.swal({
                    title: "成功!",
                    text: "您的采购清单已经保存!",
    ...
    $scope.orderProducts = function(){
        ...
        $http.post(urlFactory("/buy/order"),orderItemDtoList).then(
            function successCallback(responseData){
...
function loginCtrl($log, $http, $scope,$rootScope,toaster){
    ...
    $scope.login = function (account){
        ...
        $http.post(urlFactory("/auth/login.json"), account).then(
            function successCallback(responseData){
                ...
                if(0 != responseData.data.code){
                    ...
                    this.session_key  = responseData.data.data.session_key;
...
function buyRunningOrdersCtrl($scope, DTOptionsBuilder, $http, $log){
    ...
    //load table
    $http.get(urlFactory("/buy/unfinished_orders")).then(
        function successCallback(responseData){
            $log.log(JSON.stringify(responseData));
...
function buyFinishedOrdersCtrl($scope, DTOptionsBuilder, $http, $log){
    ...
    //load table
    $http.get(urlFactory("/buy/finished_orders")).then(
        function successCallback(responseData){
            $log.log(JSON.stringify(responseData));
            $scope.items = handleOrderInfo(responseData);
        },
...
function sellUnAcceptedOrdersCtrl($scope, DTOptionsBuilder, $http, $log){
    ...
    function loadTable(){
        //load table
        $http.get(urlFactory("/sell/unAccepted_orders")).then(
            function successCallback(responseData){
                $log.log(JSON.stringify(responseData));
                $scope.items = handleOrderInfo(responseData);
            },
    ...
    $scope.orderAction = function (orderNo, actionType){
        ...
        $http.put(urlFactory("/sell/order/"+orderNo), orderAction).then(
...
function sellUnFinishedOrdersCtrl($scope, DTOptionsBuilder, $http, $log){
    ...
    function loadTable(){
        //load table
        $http.get(urlFactory("/sell/unFinished_orders")).then(
            function successCallback(responseData){
                $log.log(JSON.stringify(responseData));
                $scope.items = handleOrderInfo(responseData);
            },
...
function sellFinishedOrdersCtrl($scope, DTOptionsBuilder, $http, $log){
    ...
    function loadTable(){
        //load table
        $http.get(urlFactory("/sell/finished_orders")).then(
            function successCallback(responseData){
                $log.log(JSON.stringify(responseData));
                $scope.items = handleOrderInfo(responseData);
            },
...
function navCtrl($scope, $http, $log,$cookies){
    $log.log("load ui menu");
    var sk = $cookies.get("session_key");
    $log.log("----" + sk);
    $scope.sell = false;
    $scope.buy = false;

    $http.get(urlFactory("/auth/accountInfo")).then(
        function successCallback(responseData){
            $log.log(JSON.stringify(responseData));
...
function urlFactory(relativeURL){
    var port = 8080;
    var ip_addr = document.location.hostname;
    var session_key = "";
    if(this.session_key != null){
            session_key = "?session_key="+this.session_key;
    }
    var url =  "http://" + ip_addr + ":"+port + relativeURL + session_key;
    alert(url)
    return url;
}
